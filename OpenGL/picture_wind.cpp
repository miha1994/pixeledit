...
void picture_wind::update (State state, float dt) {
	...
	case PWS_B:		// ксить
		...
		if (in.kb.dirs[D_UP].just_pressed || in.kb.dirs[D_DOWN].just_pressed) {		// меняем размер кисти, если нужно
			m_brush_size += (in.kb.ctrl.pressed_now ? 5 : 1) * (in.kb.dirs[D_UP].just_pressed ? 1 : -1);
			m_brush_size = Max (m_brush_size, 1);
		}
		if (in.kb.dirs[D_RIGHT].just_pressed || in.kb.dirs[D_LEFT].just_pressed) {	// меняем прозрачность, если нужно
			int a = m_color1.a;
		...
		break;
	case PWS_E:		// ластик
		if (D_CUR << pos && D_ACT) {		// стираем пиксели текущего слоя, если нужно
		...
		if (in.kb.dirs[D_UP].just_pressed || in.kb.dirs[D_DOWN].just_pressed) {		// меняем размер ластика, если нужно
	...
	case PWS_Z:		// лупа
		...
		break;
	case PWS_M:		// прямоугольное выделение
		if (D_CUR << pos && D_ACT) {
			static v2i start;
...
void picture_wind::checkout () {			// вернуться на предыдущий коммит
	--m_cur_history;
	if (m_cur_history >= 0){
